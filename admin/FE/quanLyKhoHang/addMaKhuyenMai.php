...
<div class="container">
    ...
    <?php if($noti == 3) {?><p style="text-align: center;color:red;font-size: large">Mã khuyến mại này đang chạy hoặc đã dừng không thể chỉnh sửa!</p> <?php } ?>

    <form action="../../BE/quanLyKhoHang/addMaKhuyenMai.php" method="post" onsubmit="return checkMaKhuyenMai()">
...
<script>
 function checkMaKhuyenMai() {
    ...
    var regex = /^[a-zA-Z0-9]+$/;
        if (!regex.test(makhuyenmai)) {
            alert("Mã khuyến mại chỉ gồm chữ cái không dấu và chữ số!");
            return false;
        }
