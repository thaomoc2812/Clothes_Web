...
<div class="container">
    ...
    <nav class="navbar navbar-default">
        <div class="container-fluid">
            ...
            <li><a href="choXacNhan.php?staff=<?php echo $staff?>">Chờ xác nhận</a></li>
                ...
                <li class="active"><a href="dangVanChuyen.php?staff=<?php echo $staff?>">Đang vận chuyển</a></li>
    ...
    <?php 
        ...
        $count_sql = "SELECT COUNT(*) AS total FROM donhang WHERE tinhtrang= N'Đang vận chuyển'";
            ...
            <div class="row">
                <div class="col col-md-12">
                    <table class="table table-bordered">
                        ...
                        <tbody id="datarow">
                        <?php
                            $search_donhangDaGiao= "SELECT * FROM donhang WHERE  tinhtrang= N'Đã giao đến nơi' ";
                            if($result_donhangDaGiao = mysqli_query($conn, $search_donhangDaGiao))
                            while($fetch_donhangDaGiao= mysqli_fetch_assoc($result_donhangDaGiao))
                            {
                                $iddonhang = $fetch_donhangDaGiao['id'];
                                $thoigiannhanhang = $fetch_donhangDaGiao['thoigiannhanhang'];

                                // Chuyển đổi thoigiannhanhang sang kiểu thời gian
                                $currentDate = new DateTime();
                                $receivedDate = new DateTime($thoigiannhanhang);
                                $receivedDate->modify('+3 days');

                                // Kiểm tra nếu hôm nay lớn hơn hoặc bằng 3 ngày kể từ thoigiannhanhang
                            ...
                            $search_donhang= "SELECT * FROM donhang WHERE  tinhtrang= N'Đang vận chuyển' OR tinhtrang= N'Đã giao đến nơi' ";
                            ...
                            while($fetch_donhang= mysqli_fetch_assoc($result_donhang))
                            {
                                ...
                                $sdt = $fetch_khach['sdt'];
                                $tinhtrang =$fetch_donhang['tinhtrang'];
                            ?>
                            <tr>
                                ...
                                <td>
                                    <a href="viewChiTietDonHang.php?sid=<?php echo $iddonhang ?>">Chi tiết</a></br>
                                    <?php if($tinhtrang == "Đang vận chuyển"){?><a href="../BE/daGiaoDenNoi.php?staff=<?php echo $staff?>&sid=<?php echo $iddonhang?>" class="btn btn-info">Đã giao đến nơi</a><?php }?>
                                    <?php if($tinhtrang == "Đã giao đến nơi"){?><a  style="color:darkgray">Đã giao đến nơi</a><?php }?>
