...
<body>
    ...
    <script>
        ...
        function populateTable(shiftsPerMonth) {
            const tableBody = document.getElementById('shiftsTable').getElementsByTagName('tbody')[0];
            tableBody.innerHTML = ''; // Clear any existing rows

            for (const [month, shifts] of Object.entries(shiftsPerMonth)) {
                const row = document.createElement('tr');
                const cellMonth = document.createElement('td');
                const cellShifts = document.createElement('td');

                cellMonth.textContent = month;
                cellShifts.textContent = shifts;

                row.appendChild(cellMonth);
                row.appendChild(cellShifts);
        ...
        async function renderChart() {
            ...
            populateTable(shiftsPerMonth); // Populate the table with data
